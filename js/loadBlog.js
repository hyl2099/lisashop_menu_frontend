...
function load() {
    let url = requestApi + "/paragraph/all";
    $.ajax({
        type: 'GET',
        url: url,
        timeout: 1000,
...
function loadPictures(tt) {
    var json = eval(tt); //数组
    blogListlength = json.length;
    pageBlogNum = blogListlength/blogShowNum;
    ...
    $.each(json, function (index, item) {
        ...
        divMore.className = "d-flex align-items-center mt-4";

        var pMas = document.createElement("p");
        pMas.href = "../blog-single.html";
        pMas.className = "btn btn-secondary";
        pMas.innerText = "Ver más";
        divMore.appendChild(pMas);
        // 先不加admin了
        // var pAdmin = document.createElement("p");
    ...
    console.log(blogs);
    document.getElementById("dBlogs").append();
}
